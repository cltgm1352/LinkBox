...
export function EditLinkModal({ link, categories, onClose, onSave, onSetCategory }: EditLinkModalProps) {
  ...
  const currentColor = CATEGORY_COLORS.find(
    (c) => c.key === categories.find((cat) => cat.id === link.categoryId)?.color
  );
    ...
    >
      <div className="w-full max-w-sm bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          {/* Category */}
          <div>
            <label className="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-1.5 block">カテゴリー</label>
            <div className="flex flex-wrap gap-1.5">
              <button
                onClick={() => onSetCategory(link.id, null)}
              ...
              {categories.map((cat) => {
                ...
                return (
                  <button
                    key={cat.id}
                    onClick={() => onSetCategory(link.id, cat.id)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                      active ? `${col.bg} ${col.text} ${col.border}` : "bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 border-zinc-200 dark:border-zinc-700 hover:border-zinc-300"
                    }`}
                  >
                    <span>{cat.icon}</span>
                    {cat.name}
